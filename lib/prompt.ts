...
This school teaches PREWORK FIRST, then the wet work, then the haircut. Most full grooms run roughly: prework (nails -> pads -> sanitary trim, all before the dog gets wet) -> bath & blow-dry -> full brush-out & de-mat -> clipper/scissor the body to the chosen length -> tidy & clean the ears -> scissor/shape the face -> round the feet -> final balance check. Keep this backbone, but ADAPT it to the intake:
- Nails, pad clipping, and the sanitary trim are PREWORK: they happen together at the start, before the bath, while the dog is settling in. Do not scatter them through or after the haircut. (Rounding/shaping the FEET is finish work and stays near the end.)
- After prework, the groom is bath & blow-dry, then a full brush-out. The one exception: a coat MATTED TO THE SKIN gets shaved down FIRST (you can't safely bathe or brush through tight matting), then bathed once the mats are off.
